...
// This enables real-time bidirectional voice with 1-2 second latency
// instead of the 10-15 second record->upload->transcribe->generate->TTS cycle
...
module.exports = async (req, res) => {
  ...
  try {
    const { product, script, customer_type, sales_channel, difficulty, mode, product_url_content, retry_context, voice_override } = req.body;
    ...
    if (mode === 'reversal') {
      instructions = 'You are an expert salesperson demonstrating a perfect pitch. The human plays the prospect.\n\n' +
        ...
        '- However, if the prospect is genuinely hostile after 2-3 attempts ("I said I am not interested, stop calling"), then gracefully end: "No problem at all. I appreciate your time. Have a great day." Do not be pushy to the point of harassment.';
    } else {
    ...
    // Voice selection
    const realtimeVoice = voice_override || (mode === 'reversal' ? 'onyx' : 'shimmer');
    ...
    if (!sessionResponse.ok) {
      ...
      return res.status(500).json({
        error: 'OpenAI session creation failed (' + sessionResponse.status + '): ' + errText.substring(0, 500)
      });
    }
